/* zpaqd v7.15 - ZPAQ compression development tool - Aug. 17, 2016.
...
Compute file sizes and SHA-1 checksums.
  ...
  zpaqd e inkey|"" outkey|"" input output

Change the encryption key from input to output, where input is encrypted
with inkey and output is encrypted with outkey. If either key is "" then
the file is not encrypted. The encryption is compatible with with
zpaq -key, but the input and output need not be zpaq archives. For example:

  zpaqd e "" key1 file file1           Encrypt to file1 with key1
  zpaqd e key1 key2 file1 file2        Change key to key2 in file2
  zpaqd e key2 "" file2 file3          Decrypt to file3

To compile: g++ -O3 -s -static -msse2 zpaqd.cpp libzpaq.cpp -o zpaqd
For MSVC:   cl /O2 /EHsc zpaqd.cpp libzpaq.cpp advapi32.lib
...
void list(const char* archive) {
  ...
  while (d.findBlock(&mem)) {
    ...
    while (d.findFilename(&filename)) {
      ...
      printf(" %s %s -> %1.0f\n", filename.s.c_str(),
             comment.s.c_str(), double(in.tell()-d.buffered()-offset));
      offset=in.tell()-d.buffered();
...
// encrypt: zpaqd e inkey outkey in out
// If inkey or outkey are "" then in or out is not encrypted.
void encrypt(int argc, char** argv) {
  if (argc!=6) usage();

  // Open files
  FILE* in=fopen(argv[4], "rb");
  if (!in) return (void)perror(argv[4]);
  FILE* out=fopen(argv[5], "wb");
  if (!out) return (void)perror(argv[5]);

  // Stretch passwords
  char key[2][32]={{0}}, salt[2][32]={{0}};
  for (int i=0; i<2; ++i) {  // inkey, outkey
    libzpaq::SHA256 sha256;
    if (argv[i+2][0]) {  // not ""
      if (i==0) {  // read salt from encrypted input
        if (fread(salt[0], 1, 32, in)!=32)
          return (void)fprintf(stderr, "%s: no salt\n", argv[4]);
      }
      else {  // write new salt to encrypted output
        libzpaq::random(salt[1], 32);
        fwrite(salt[1], 1, 32, out);
      }
      for (int j=0; argv[i+2][j]; ++j)
        sha256.put(argv[i+2][j]);
      libzpaq::stretchKey(key[i], sha256.result(), salt[i]);
    }
  }

  // Encrypt
  libzpaq::AES_CTR aes0(key[0], 32, salt[0]);  // input
  libzpaq::AES_CTR aes1(key[1], 32, salt[1]);  // output
  int n;  // bytes read
  int64_t off=32;  // file offset including salt
  const int BUFSIZE=4096;
  char buf[BUFSIZE];
  while ((n=fread(buf, 1, BUFSIZE, in))>0) {
    if (argv[2][0]) aes0.encrypt(buf, n, off);
    if (argv[3][0]) aes1.encrypt(buf, n, off);
    if (int(fwrite(buf, 1, n, out))!=n) return (void)perror(argv[5]);
    off+=n;
  }
  fclose(out);
  fclose(in);
...
void usage() {
  printf(
    "zpaqd v7.15 ZPAQ development tool, " __DATE__ "\n"
    ...
    "To encrypt:      zpaqd e in_key|\"\" out_key|\"\" input output\n"
...
int main(int argc, char** argv) {
  ...
  if (argc<3) usage();
  char cmd=argv[1][0];
  if (strchr("acldrtse", cmd)==0) usage();
  ...
  switch (cmd) {
    ...
    // Run or trace
    case 'r':  // argv[i]... = {h|p} [input [output]]
    case 't':  // argv[i]... = {h|p} [N|xN]...
    {
      ...
      for (int i=1; segments && i!=firstblock+blocks && de.findBlock(&memory);
        ...
        while (segments && de.findFilename(&filename)) {
          ...
          if (i<firstblock) {
            ...
            --segments;
            double inStart=in.tell()-de.buffered();
            do {
              printf("%s %s %1.0f -> %1.0f \r", filename.s.c_str(),
                     comment.s.c_str(), in.tell()-de.buffered()-inStart,
                     sha1.size());
              fflush(stdout);
            ...
            printf(" %s %s %1.0f -> %1.0f", filename.s.c_str(),
                   comment.s.c_str(), in.tell()-de.buffered()-inStart, size);
    ...
    // Encrypt inkey|"" outkey|"" input output
    case 'e':
      encrypt(argc, argv);
      break;
