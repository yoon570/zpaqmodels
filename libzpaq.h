...
public:
  ...
  std::tuple<int, std::vector<int>> predict();        // probability that next bit is a 1 (0..4095)
...
private:
  ...
  std::tuple <int, std::vector<int>> predict0();       // default
...
public:
  ...
  void init();
  std::vector <float> compress(int c);  // c is 0..255 or EOF
  int stat(int x) {return pr.stat(x);}
